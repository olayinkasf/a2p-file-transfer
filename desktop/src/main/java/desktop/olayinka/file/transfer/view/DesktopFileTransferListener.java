 ...
 */
public class DesktopFileTransferListener extends AbstractTransferListener {
    ...
    public DesktopFileTransferListener(AppContext mContext, Device device) {
        super(device);
        mProgressBar.setForeground(AppSettings.APP_COLOR);
        mTransferProvider = mContext.getJDBCHelper().getTransferProvider();
    }

    @Override
    public void registerProgress(long transferred) {
        super.registerProgress(transferred);
        mStatusLabel.setText("Transferred " + mTransfer.getTransferredSize() + "/" + mTransfer.getExpectedSize() + " bytes");
        mProgressBar.setValue((int) ((mTransfer.getTransferredSize() * 100) / mTransfer.getExpectedSize()));

    }

    @Override
    public void registerName(String name) {
        super.registerName(name);
        mTitleLabel.setText(mTransfer.getFile().getName());
    }
    ...
    public void registerFinished() {
        ...
        if (!mTransfer.getStatus().equals(Transfer.Status.FINISHED)) {
            //TODO
            System.out.println();
        }
    }
